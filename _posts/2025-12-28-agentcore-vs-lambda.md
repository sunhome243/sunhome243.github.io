...
image: https://d2908q01vomqb2.cloudfront.net/fc074d501302eb2b93e2554793fcaf50b3bf7291/2024/11/20/Agentic-AI-Workload-AWS-Lambda-1260x630.png
optimized_image:
category: "blog"
tags:
  - aws
...
- **Simple AI workflow**: P99 6 LLM calls per tutoring interaction, not complex multi-agent orchestration
...
## Our Use Case: AI-Powered Tutoring System

We're building an AI tutoring platform for ADHD students. The core AI workflow:

1. Receive student context (assignments, learning history)
2. Make 1-2 LLM API calls
3. Return personalized learning content

**Average execution time: ~30 seconds**
**With database calls: ~32 seconds**
...
- LLM inference: ~25-28 seconds (actual compute on LLM provider side)
- I/O wait on our side: ~2-4 seconds
- Database calls: ~2 seconds

We're on the **low end of I/O-heavy workloads**. Our agent makes 1-2 calls, not 10-20.
...
For our projected volume (early-stage startup), Lambda's generous free tier meant **months of zero compute costs**. The 2-4 seconds of I/O savings per request couldn't offset this.
...
- **P99 response: 48 seconds**
- **Maximum observed: 72 seconds**
...
Lambda with its free tier, mature ecosystem, and proven reliability was the pragmatic choice.
...
- AgentCore shines for **I/O-heavy agents** (30-70% wait time)—ours was only ~10%
- **Free tier economics** matter enormously for early-stage products
- **8-hour runtime** is powerful, but only if you need it
- **Built-in memory** is valuable for cross-device sync—not for local-first apps
